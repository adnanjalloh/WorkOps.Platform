...
| Privilege escalation | Implemented for the bounded membership lifecycle | Central permissions, contributor/viewer invitation limits, owner-only privileged role grants, administrator target limits, and fresh caller-role checks after a workspace lock; [contract and tests](member-management.md). Provider administration remains external |
| Concurrent removal of every owner | Implemented for API membership writes | Shared workspace-row lock, fresh actor/target checks, last-active-owner check, expected versions, and deterministic two-owner race tests; arbitrary operator SQL must follow the same invariant |
| Revoked membership used by a waiting request | Implemented for membership mutations | Invitations, role changes, and deactivation recheck activity/role after their lock. Subsequent normal requests query current membership; unrelated in-flight requests and queued messages are not globally cancelled |
